...
sessionPersistenceReady
    .then(() => {
        ...
        if (active?.id) {
            ...
            log("Restored active recording session:", sessionId);
        }
    })
...
async function notifyTabsStart(resume = false) {
    ...
    try {
        ...
        console.log("Recording flag sent to tab:", activeRecordingTabId);
    } catch (err) {
        console.warn("Failed to toggle recording on tab:", activeRecordingTabId, err);
    }
}
...
async function notifyTabsStop() {
    ...
    try {
        ...
        console.warn("Failed to stop recording on tab:", activeRecordingTabId, err);
    }
}
...
export async function stopRecordingSession() {
    ...
    console.log("\"S Event types:", recordedEvents.map((e) => e.type));
